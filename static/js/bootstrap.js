...
function loadHandler() {
    canvasElem = document.getElementById("CanvasElement");

    if (canvasElem.getContext) {
        canvasCtx = canvasElem.getContext("2d");

        exampleMain(); // call main example function
    } else {
...
function exampleMain() {
    window.setInterval(drawDropsIntervalHandler, 40);
...
function drawDrops() {
    ...
    for (var i = 0; i < dropsArray.length; i++) {
        ...
        canvasCtx.fillStyle = "rgba(" + drop.color.r + "," + drop.color.g + ","
                + drop.color.b + "," + drop.color.alpha + ")";

        canvasCtx.beginPath();
        canvasCtx.arc(drop.centerX, drop.centerY, drop.radius, 0, 2 * Math.PI,
                false);
        canvasCtx.fill();
    }
}
